...
Questa funzione è tornata utile in fase di training (v.sez.TODO) in
quanto ha reso possibile specificare l'area dei documenti contenuti
nelle immagini di training in maniera molto precisa, evitando ad esempio
la l'inclusione dei polpastrelli che in alcuni casi possono influenzare
il comportamento della rete. Per effettuare l'OCR abbiamo optato per la
libreria tesseract, resa disponibile in python tramite il modulo wrapper
...
La logica della rete \mrcnn\ consiste nell'esecuzione di due fasi. Nella
...
Si rimanda infine al link dell'applicativo web utilizzato per la
...
Segue la struttura del dataset da noi utilizzato.
...
problema.\\
La cartella testing contiene invece foto da utilizzare quando si volesse
eseguire un test manuale dei risultati del training in Colab.
...
web, si andrà quindi a descrivere la progettazione di una web
application. Il nostro sistema sarà dunque suddiviso in tre macro-parti:

\begin{itemize}
    \item Il \textit{frontend}, implementato tramite tecnologie web
        JavaScript, HTML e CSS
    \item Il \textit{backend}, implementato in python tramite il
        framework flask per l'erogazione di servizi web.
...
rappresenta le classi utilizzate durante il training della rete (v.
notebook Jupyter). \texttt{documentCNN} si riferisce al modulo di core
business da noi sviluppato.
